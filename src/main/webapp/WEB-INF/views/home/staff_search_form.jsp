...
<div id="staffContainer">
	<table id="tbl-board" class="table table-hover">
		...
		<c:forEach items="${list}" var="staff">
			<tr>
				<td class="align-middle">${staff.no}</td>
				<td class="align-middle">${staff.staffName}</td>
				<td class="align-middle">${staff.gender}</td>
				<td class="align-middle">${staff.departmentName}</td>
				<td class="align-middle">${staff.graduateDay}</td>
				<td><button value="${staff.staffNo}" id=modifyDelete type="button" class="btn btn-secondary float-right">수정/삭제</button></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
...
<script>
	...
	$("th").click((e) => {
		...
		$.ajax({
			url: "${pageContext.request.contextPath}/staff/searchStaff.do",
			method: "POST",
			data: searchStr,
