...
<body>
			...
			<tr>
				...
				<th>주민번호</th>
				<td><input type="text" style="width:35%;" id="jumin1" required="required" value="${fn:substring(staff.juminNo,0,6) }"> - <input type="password" style="width:35%;" id="jumin2" value="${fn:substring(staff.juminNo,6,13) }"></td>
				<th>부서</th>
				<td>
					<select style="width:100%;" id="department">
						...
						<option value="하이테크사업부" <c:if test="${staff.departmentCode eq '하이테크사업부' }">selected</c:if>>하이테크사업부</option>
						...
						<option value="기업부설연구소" <c:if test="${staff.departmentCode eq '기업부설연구소' }">selected</c:if>>기업부설연구소</option>
			...
			<tr>
				<th>학력</th>
				<td>
					<input type="checkbox" name="school_code" value="고졸" <c:if test="${staff.schoolCode eq '고졸' }">checked</c:if>><label class="school">고졸</label>
					<input type="checkbox" name="school_code" value="전문대졸" <c:if test="${staff.schoolCode eq '전문대졸' }">checked</c:if>><label class="school">전문대졸</label>
					<input type="checkbox" name="school_code" value="일반대졸" <c:if test="${staff.schoolCode eq '일반대졸' }">checked</c:if>><label class="school">일반대졸</label>
				</td>
				<th>기술</th>
				<td colspan="3">
					<input type="checkbox" name="skill" value="JAVA" <c:if test="${staff.skillCode eq '31' }">checked</c:if>><label>Java</label>
					<input type="checkbox" name="skill" value="JSP" <c:if test="${staff.skillCode eq '32' }">checked</c:if>><label>JSP</label>
					<input type="checkbox" name="skill" value="ASP" <c:if test="${staff.skillCode eq '33' }">checked</c:if>><label>ASP</label>
					<input type="checkbox" name="skill" value="PHP" <c:if test="${staff.skillCode eq '34' }">checked</c:if>><label>PHP</label>
					<input type="checkbox" name="skill" value="DELPHI" <c:if test="${staff.skillCode eq '35' }">checked</c:if>><label>Delphi</label>
				</td>
			</tr>
			<tr>
				<th>졸업일</th>
